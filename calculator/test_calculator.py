...
def run_tests():
    ...
    test_cases = [
        ...
        (
            "1 * 2 / 3",
            0.6666666666666666,
        ),  # Test multiple operators with different precedence
    ]
    ...
    for expression, expected in test_cases:
        try:
            ...
            assert result == expected or (
                isinstance(expected, str) and expected in str(result)
            ), f"Test failed for '{expression}'. Expected {expected}, got {result}"
        except ValueError as e:
            print(f"Expression: '{expression}', Error: {e}, Expected error: {expected}")
            assert isinstance(expected, str) and expected in str(e), (
                f"Test failed for '{expression}'. Expected error containing '{expected}', got '{e}'"
            )
